...
try{
    ...
    $insertar = $conexion->prepare("insert into metodo_pago (
        codigo_metodo, 
        descripcion)
        values (:A,:B)");
...
if ($error==23000){
  echo '<script>confirmar=confirm("Ese codigo de motivo ya existe");
          if (confirmar)
            window.location.href="insertar_metodo_pago-forma.php";</script>';
          echo "<a href= insertar_metodo_pago-forma.php>Volver</a>";
}else{
