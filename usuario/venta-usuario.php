...
<head>
    ...
    <link rel="shortcut icon" href="./img/iconos/propiedad.png" type="image/x-icon">
...
<body>
    ...
    <div class="vender__principal">
        <div class="vender__principal-textos">
            <h1 class="vender__principal-tittle">Bienvenido a comprar propiedades</h1>
            <h3 class="vender__principal-subtittle">En colviviendas nos aseguramos de que tenga una buena experiencia en
                la compra de propiedades</h3>
        </div>
    </div>
    ...
    <div class="container-vender">
        ...
        if (!empty($_GET['cod']) && $_GET['cod'] = 'buscar') {
            $vtipo = filter_var($_POST["tipo"]);
            $vbarrio = filter_var($_POST["barrio"]);
            $vciudad = filter_var($_POST["ciudad"]);
            $vbanos = filter_var($_POST["bano"]);
            $vhabitaciones = filter_var($_POST["habitacion"]);
        ...
        if (!empty($vciudad) || !empty($vtipo) || !empty($vbarrio) || !empty($vbanos) || !empty($habitaciones)) {

            $matriz = mysqli_query($dbhandle, "SELECT a.codigo_propiedad, a.direccion, a.foto, b.descripcion as estado, c.nombre, c.apellido, d.descripcion as metodo_pago, e.descripcion as ciudad, f.descripcion as barrio, a.precio, g.descripcion as modelo, a.fecha_registro, h.descripcion as tipo_propiedad, a.nro_banos, a.nro_habitaciones, a.nro_garajes, a.nro_metros, a.edad, i.descripcion as destinacion 
            FROM propiedad a
            JOIN estado b ON a.estado = b.codigo_estado 
            JOIN persona c ON a.propietario = c.identificacion 
            JOIN metodo_pago d ON a.metodo_pago = d.codigo_metodo 
            JOIN ciudad e ON a.ciudad = e.codigo_ciudad 
            JOIN barrio f ON a.barrio = f.codigo_barrio 
            JOIN modelo g ON a.modelo = g.codigo_modelo 
            JOIN tipo_propiedad h ON a.tipo_propiedad = h.codigo_tipo 
            JOIN destinacion i ON a.destinacion = i.codigo_destinacion 
            WHERE a.destinacion != '20' 
            AND a.estado != 'E2' 
            AND a.estado != 'E3' 
            AND (a.tipo_propiedad = '" . $vtipo . "' OR a.nro_habitaciones = '" . $vhabitaciones . "' OR a.barrio = '" . $vbarrio . "' OR a.ciudad = '" . $vciudad . "');");


            $vregistros = mysqli_num_rows($matriz);

            if ($vregistros == 0) {
                echo "Lo lamentamos, no se encontraron propiedades con esas caracteristicas :(";
            }

        } else {
            $matriz = mysqli_query($dbhandle, "SELECT a.codigo_propiedad, a.direccion, a.foto, b.descripcion as estado, c.nombre, c.apellido, d.descripcion, e.descripcion as ciudad, f.descripcion as barrio, a.precio, g.descripcion as modelo, a.fecha_registro, h.descripcion, a.nro_banos, a.nro_habitaciones, a.nro_garajes, a.nro_metros, h.descripcion as tipo_propiedad, a.edad, i.descripcion as destinacion FROM propiedad a, estado b, persona c, metodo_pago d, ciudad e, barrio f, modelo g, tipo_propiedad h, destinacion i where a.estado = b.codigo_estado and a.propietario = c.identificacion and a.metodo_pago = d.codigo_metodo and a.ciudad = e.codigo_ciudad and a.barrio = f.codigo_barrio and a.modelo = g.codigo_modelo and a.tipo_propiedad = h.codigo_tipo and a.destinacion = i.codigo_destinacion and a.destinacion != '20' and a.estado != 'E2' and a.estado != 'E3';");
